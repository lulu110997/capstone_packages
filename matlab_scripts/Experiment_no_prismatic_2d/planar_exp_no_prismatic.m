...
scale_lengths = 1.25;
L1 = Link('d',0,'a',scale_lengths*0.6127,'alpha', 0);
...
q0 = R.ikcon(T);                                            % Solve joint angles to achieve first waypoint
...
R.plot(qMatrix,'trail','r-', 'fps', fps)
%%
...
%             arm_weight = min_allowable_weight + ((arm_weight-0.01)/(15-0.01))*(0.99-min_allowable_weight);
        ...
        for j=1:R.n
            if j == 1 || j == 4
                ...
                secondary_weight(1,j) = platform_weight;
            end
        end
