...
class UrVoguiKdl(object):
    '''
    To do:
    - Analytical calculation for gradient manipulation
    - Manipulability gradient fails @ starting joint pos. Rotational part is 0 due to det(JJ') returning a negative value
    - Which Jacobian to use for maximising manipulability? Should be the whole system since we can have a motion of the
      mobile platform where the EE does not move 
    - Figure out how to obtain arm jacobian and system jacobian without having
    to multiply by a rotation matrix. Related to attaching a frame to segx to 
    match jacobian from Corke's RTB
    ...
    def manipulability_gradient(self, q, ee_frame=False):
        ...
        This is then used to compute the manipulability at this specific joint configuration. This
        function computes the manipulability gradient of the manipulator since the platform is used
        mainly to satisfy a primary task
        '''
        # import time
        # start = time.time()
...
if __name__ == '__main__': 
    q0 = np.array([0, 0, 0, -0.5230, 1.2215, 2.9665, 1.5705, 0])
    ob = UrVoguiKdl(urdf_file='/home/louis/catkin_ws/src/custom_ur_control/rb_vogui_generated.urdf')
    ob.fkine(q0)
    ob.Jacob(q0)
    ob.Jacob(q0, True)
