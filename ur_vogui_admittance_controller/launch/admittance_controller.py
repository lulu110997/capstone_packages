...
class AdmittanceController():
    ...
    def __init__(self):
        ...
        self.__nr_joints = 9
        self.__task_space = 6
        self.__jacob0 =  np.zeros((self.__task_space,self.__nr_joints)) # Jacobian matrix in world frame
        self.__jacobe =  np.zeros((self.__task_space,self.__nr_joints)) # Jacobian matrix in ee frame relative to world
        self.__ft = np.zeros((self.__nr_joints,1)) # Force exerted on the ee (from joystick)
        self.__q = np.zeros((self.__nr_joints,1)) # Current joint states
    ...
    def robot_state_callback(self, msg):
        ...
        in reverse order
        '''
        self.__q[3] = msg.position[2]
        self.__q[4] = msg.position[1]
        self.__q[5] = msg.position[0]
        self.__q[6] = msg.position[3]
        self.__q[7] = msg.position[4]
        self.__q[8] = msg.position[5]
    ...
    def jacob0_callback(self, msg):
        ...
        for i in range(self.__task_space):
            for j in range(self.__nr_joints):
    ...
    def __jacob_inv_dls(self, jacob):
        ...
        dls = np.matmul(j_transpose, jacob) + (0.1*0.1*np.eye(self.__nr_joints))
    ...
    def main(self):
        ...
        q_desired = np.array([[0, 0, 0, -0.87266, -1.2217, 1.2217, 0, 0, 0]]).transpose() # The desired joint setpoint
        ...
        ka = np.zeros((self.__task_space,self.__nr_joints)) # Virtual spring stiffness of the admittance controller
        kp = 0.15 # Error constant
        np.fill_diagonal(ka, 0.5) # Populate the virtual spring matrix

        dt = 0.1 # Needs to be the same rate that the HARDWARE controller is running at

        # [-0.06340057188217522, 0.00012566847434230884, 0.5545] # Robot base location
        rate_limiter = rospy.Rate(25) # 25Hz or needs to be the same rate that the HARDWARE is running at
        ...
        while not rospy.is_shutdown():
            ...
            try:
                ...
                jacobe_dls = np.linalg.pinv(self.__jacobe)
                # print('dls ', jacobe_dls.shape)
                # print('jacoe ', self.__jacobe.shape)
                ...
                base_vel = Twist()
                base_vel.angular.z = q[0]
                base_vel.linear.y = q[1]
                base_vel.linear.x = q[2]
